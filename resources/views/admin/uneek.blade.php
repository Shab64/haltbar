				...
				<div class="card">
					<div class="card-body">
						...
						<div class="table-responsive">
							<table id="report-table2" class="table mb-0">
								...
								<tbody>
									<?php
									if (!empty($uneek)) {
										foreach ($uneek as $k => $product) { ?>
											<tr>
												<td class="align-middle">
													<img src="<?= $product['Model_Small_Image'] ?>" alt="contact-img" title="contact-img" class="rounded mr-3" height="48" />
												...
												<td class="table-action">
													<!-- </?= site_url('admin/products/product-details/' . $product['id']) ?> -->
													<a href="javascript:void(0)" onclick="event.preventDefault();
                                                    document.getElementById('input-form{{ $k }}').submit();" class="btn btn-icon btn-outline-primary"><i class="feather icon-eye"></i></a>
													<form action="{{ Route('admin.viewIndividual') }}" id="input-form{{ $k }}" method="POST" class="d-none">
														@csrf
														<input type="text" hidden name="type" value="uneek" />
                            ...
                            <div class="w-100">{{ $uneek->links() }}</div>
                        </div>
                    </div>
				</div>
			</div>
